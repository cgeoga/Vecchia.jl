...
struct VecchiaPrediction{M,P,F}
  n::Int64
  meanfun::M
  kernel::F
...
function VecchiaPrediction(va::SingletonVecchiaApproximation{M,P,F},
                           pred_pts::Vector{P};
...
function predict(vp::VecchiaPrediction{M,P,F}, cov_params, mean_params; 
                 conditional_simulate=false) where{M,P,F}
...
function predict(vp::VecchiaPrediction, params; conditional_simulate=false)
  predict(vp, params, params; conditional_simulate=conditional_simulate)
end

function predict(vp::VecchiaPrediction, params::Parameters; 
                 conditional_simulate=false)
