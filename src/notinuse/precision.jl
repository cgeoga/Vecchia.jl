...
function precisionmatrix(V::VecchiaApproximation{D,F}, params::Vector{T}; 
                         issue_warning=true) where{D,F,T}
  checkthreads()
  if issue_warning
    @warn "Note that this is the precision matrix for your data enumerated \
    according to the permutation of the VecchiaApproximation structure, so if you plan to \
